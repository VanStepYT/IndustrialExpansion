...
public class SolarGenComponent extends GuiWidget {
    SolarReactorTile machine;
    Vec2i offset;
    boolean vertical;
    public SolarGenComponent(Box2i box, SolarReactorTile machine, Vec2i offset, boolean vertical) {
        super(box);
   ...
   @OnlyIn(Dist.CLIENT)
    public void drawBackground(PoseStack matrix, int mouseX, int mouseY, float partialTicks) {
        float generation = this.machine.getGeneration();
        if (generation >= 0.0F) {
            Box2i box = this.getBox();
            int width = box.getWidth();
            int height = box.getHeight();
            float lvl = (float)(this.vertical ? height : width) * Math.min(1.0F, generation / this.machine.getMaxGeneration());
            if (lvl <= 0.0F) {
                return;
            }
