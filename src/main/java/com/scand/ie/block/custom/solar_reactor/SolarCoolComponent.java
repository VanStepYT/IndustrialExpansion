...
public class SolarCoolComponent extends GuiWidget {
    SolarReactorTile machine;
    Vec2i offset;
    boolean vertical;
    public SolarCoolComponent(Box2i box, SolarReactorTile machine, Vec2i offset, boolean vertical) {
        super(box);
    ...
    public void drawBackground(PoseStack matrix, int mouseX, int mouseY, float partialTicks) {
        float cooling = this.machine.getCooling();
        if (cooling >= 0.0F) {
            Box2i box = this.getBox();
            int width = box.getWidth();
            int height = box.getHeight();
            float lvl = (float)(this.vertical ? height : width) * Math.min(1.0F, cooling / this.machine.getMaxHeat());
            if (lvl <= 0.0F) {
                return;
            }
